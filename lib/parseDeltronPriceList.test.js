...
const { parsePriceListRows, PRICE_SENTINELS_NO_PRICE, stripWrappingQuotes } = require('./parseDeltronPriceList');

test('stripWrappingQuotes quita un par de comillas que envuelven todo el valor', () => {
  assert.equal(stripWrappingQuotes('"deltron"'), 'deltron');
});

test('stripWrappingQuotes no toca un valor sin comillas envolventes', () => {
  assert.equal(stripWrappingQuotes('deltron'), 'deltron');
  assert.equal(stripWrappingQuotes('marca "con comillas" adentro'), 'marca "con comillas" adentro');
});

test('stripWrappingQuotes maneja vacío/undefined sin lanzar', () => {
...
test('PRICE_SENTINELS_NO_PRICE incluye las 5 magnitudes "todo nueves" verificadas contra el archivo real', () => {
  assert.deepEqual(
    [...PRICE_SENTINELS_NO_PRICE].sort((a, b) => a - b),
    [9.0, 99.0, 999.0, 9999.0, 9999999.99]
  );
});
...
for (const sentinel of [9.0, 99.0, 999.0, 9999.0]) {
  test(`parsePriceListRows omite y cuenta el precio centinela ${sentinel.toFixed(2)}`, () => {
    // Verificado contra el archivo real: este valor aparece repetido en
    // productos totalmente distintos entre sí (ver comentario junto a
    // PRICE_SENTINELS_NO_PRICE), lo que descarta que sea coincidencia de
    // precio real.
    const csvText = `"monitor plano 27","montest_sentinela","producto de prueba","10","${sentinel.toFixed(2)}", ,"","W","marca test"`;
...
test('parsePriceListRows reconstruye una categoría con coma interna sin comillas (10 columnas en vez de 9)', () => {
  // Verificado contra el archivo real de Deltron (2026-08-13): categorías
  // como "ACC, MUEBLES DE COMPUTO" salen SIN comillas en el export, así que
  // la coma interna las parte en dos columnas CSV. Debe reconstruirse en
  // vez de contarse como malformada -- de lo contrario se pierden todas las
  // filas de cualquier categoría con coma en el nombre (varias del mapeo
  // real: ACC, MUEBLES DE COMPUTO / AUDIO, ACCESORIOS DE / UPS, ACCESORIOS).
  const csvText = 'acc, muebles de computo,"acbolsaterosmun","bolsa teros mundial 50 x 40",>20,3.50, ,"","Y","deltron"';
  const { rows, skippedMalformed } = parsePriceListRows(csvText);
  assert.equal(skippedMalformed, 0);
  assert.equal(rows.length, 1);
  assert.deepEqual(rows[0], {
    category: 'ACC, MUEBLES DE COMPUTO',
    codigo: 'acbolsaterosmun',
    descripcion: 'bolsa teros mundial 50 x 40',
    stock: { qty: null, status: 'in_stock' },
    priceUsd: 3.5,
    marca: 'deltron',
  });
...
test('parsePriceListRows limpia comillas literales que quedan pegadas a la marca', () => {
  // Verificado contra el archivo real: 1891 de 1894 filas reales traen
  // `marca` con comillas literales pegadas, por una interacción entre
  // relax_quotes y comillas mal formadas más adelante en el archivo real.
  // Este test usa el escape CSV estándar ("" dentro de un campo citado ==
  // una comilla literal) para que el valor ya parseado de `marca` llegue
  // con comillas pegadas, igual que en el archivo real.
  const csvText =
    '"monitor plano 27","montest0006","monitor de prueba",">20","150.00", ,"","W","""marca con comillas"""';
  const { rows } = parsePriceListRows(csvText);
  assert.equal(rows.length, 1);
  assert.equal(rows[0].marca, 'marca con comillas');
});
